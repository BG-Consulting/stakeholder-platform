        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={(e) => handleChange("name", e.target.value)} placeholder="e.g. Minister of Energy, Republic of Kenya" className="w-full px-3 py-2 text-sm text-white placeholder-gray-500 outline-none" style={inputStyle} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select value={form.type} onChange={(e) => { const newType = e.target.value as Stakeholder["type"]; handleChange("type", newType); handleChange("category", TYPE_TO_CATEGORY[newType]); }} className="w-full px-3 py-2 text-sm text-white outline-none" style={inputStyle}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <input type="text" value={form.key_position_1} onChange={(e) => handleChange("key_position_1", e.target.value)} placeholder="Key position 1" className="w-full px-3 py-2 text-sm text-white placeholder-gray-500 outline-none" style={inputStyle} />
              <input type="text" value={form.key_position_2} onChange={(e) => handleChange("key_position_2", e.target.value)} placeholder="Key position 2" className="w-full px-3 py-2 text-sm text-white placeholder-gray-500 outline-none" style={inputStyle} />
              <input type="text" value={form.key_position_3} onChange={(e) => handleChange("key_position_3", e.target.value)} placeholder="Key position 3" className="w-full px-3 py-2 text-sm text-white placeholder-gray-500 outline-none" style={inputStyle} />
            </div>
          </div>
          <div>
...
const CATEGORIES: StakeholderCategory[] = ["Government & Regulatory", "Private Sector", "Civil Society & NGOs", "Media & Communications", "Academic & Research", "International Organizations & Donors"];

const CATEGORY_COLORS: Record<StakeholderCategory, string> = { "Government & Regulatory": "#1d4ed8", "Private Sector": "#0f766e", "Civil Society & NGOs": "#7e22ce", "Media & Communications": "#b45309", "Academic & Research": "#0369a1", "International Organizations & Donors": "#0d9488" };

const TYPE_LABELS: Record<Stakeholder["type"], string> = { government: "Government", private: "Private Sector", ngo: "NGO", media: "Media", academic: "Academic", international: "International" };

const TYPE_COLORS: Record<Stakeholder["type"], string> = { government: "#1d4ed8", private: "#0f766e", ngo: "#7e22ce", media: "#b45309", academic: "#0369a1", international: "#0d9488" };
...
function ModeBadge({ mode }: { mode: ResultMode }) {
  if (mode === "demo") {
    return (<span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold tracking-widest uppercase" style={{ background: "rgba(234, 179, 8, 0.12)", color: "#fbbf24", border: "1px solid rgba(234, 179, 8, 0.35)", borderRadius: "2px" }}><span className="w-1.5 h-1.5 rounded-full" style={{ background: "#fbbf24" }} />Demo</span>);
  }
  return (<span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold tracking-widest uppercase" style={{ background: "rgba(22, 163, 74, 0.12)", color: "#4ade80", border: "1px solid rgba(22, 163, 74, 0.35)", borderRadius: "2px" }}><span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "#4ade80" }} />Live</span>);
}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {Array.from({ length: 10 }).map((_, i) => (<div key={i} className="w-2.5 h-2.5 rounded-sm" style={{ background: i < score ? (i < 4 ? "var(--crimson)" : i < 7 ? "#f59e0b" : "#ef4444") : "rgba(255,255,255,0.08)" }} />))}
      </div>
...
function ContactInfo({ contact }: { contact?: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-1">
      {isEmail ? (<svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "#60a5fa" }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>) : (<svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "#60a5fa" }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101M10.172 13.828a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>)}
...
function StakeholderCard({ stakeholder }: { stakeholder: Stakeholder }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-5 rounded" style={{ background: "var(--navy-800)", border: "1px solid rgba(255,255,255,0.06)", transition: "border-color 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.borderColor = "rgba(204, 41, 54, 0.3)")} onMouseLeave={(e) => (e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)")}>
      <div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-2">
          ...
          {stakeholder.category && (<span className="px-2 py-0.5 rounded text-[10px] font-medium tracking-wide" style={{ background: `${CATEGORY_COLORS[stakeholder.category] ?? "#555"}20`, color: CATEGORY_COLORS[stakeholder.category] ?? "#999", border: `1px solid ${CATEGORY_COLORS[stakeholder.category] ?? "#555"}30` }}>{stakeholder.category}</span>)}
          {stakeholder.sources && stakeholder.sources.length > 0 && stakeholder.sources[0] === "Uploaded document" && (<span className="px-2 py-0.5 rounded text-[10px] font-medium tracking-wide" style={{ background: "rgba(99, 102, 241, 0.15)", color: "#818cf8", border: "1px solid rgba(99, 102, 241, 0.3)" }}>From document</span>)}
        </div>
      </div>
      ...
      <div>
        ...
        <ul className="flex flex-col gap-1.5">
          {stakeholder.key_positions.map((pos, i) => (<li key={i} className="flex gap-2 text-sm" style={{ color: "#b0bac8" }}><span style={{ color: "var(--crimson)", marginTop: "1px" }}>{"\u203A"}</span>{pos}</li>))}
        </ul>
      </div>
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }} />
      <div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (uploadedFile) {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          docStakeholders = data.stakeholders;
        }
      } catch { /* non-fatal */ }
      setUploadedFile(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--navy-900)" }}>
      ...
      <main className="flex-1 px-6 py-10 max-w-7xl mx-auto w-full">
        ...
        {uploadLoading && !stakeholders && (<div className="text-center py-16"><div className="inline-block w-8 h-8 rounded-full border-2 animate-spin mb-4" style={{ borderColor: "#818cf8", borderTopColor: "transparent" }} /><p className="text-sm" style={{ color: "#8892a4" }}>Extracting stakeholders from document{"\u2026"}</p></div>)}

        {loading && !stakeholders && (<div className="text-center py-16"><div className="inline-block w-8 h-8 rounded-full border-2 animate-spin mb-4" style={{ borderColor: "var(--crimson)", borderTopColor: "transparent" }} /><p className="text-sm" style={{ color: "#8892a4" }}>Searching and analysing stakeholders for{" "}<strong className="text-white">{lastQuery?.sector}</strong> in{" "}<strong className="text-white">{lastQuery?.region}</strong>{"\u2026"}</p></div>)}

        {error && (<div className="px-5 py-4 rounded text-sm mb-8" style={{ background: "rgba(204, 41, 54, 0.1)", border: "1px solid rgba(204, 41, 54, 0.3)", color: "#f87171" }}><strong>Error:</strong> {error}</div>)}
        ...
        {stakeholders && (
          <>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-6">
              ...
              {CATEGORIES.map((cat) => { const count = categoryCounts[cat] || 0; if (count === 0) return null; const isActive = activeCategory === cat; const color = CATEGORY_COLORS[cat]; return (<button key={cat} onClick={() => setActiveCategory(isActive ? null : cat)} className="px-3 py-1.5 rounded text-xs font-semibold transition-all duration-150" style={{ background: isActive ? `${color}30` : "rgba(255,255,255,0.05)", color: isActive ? color : "#8892a4", border: `1px solid ${isActive ? `${color}60` : "rgba(255,255,255,0.08)"}` }}>{cat} ({count})</button>); })}
            </div>

            {viewMode === "list" && (<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" style={{ animation: "fadeIn 0.25s ease-out" }}>{(filteredStakeholders ?? []).sort((a, b) => b.influence_score - a.influence_score).map((s, i) => (<StakeholderCard key={`${s.name}-${i}`} stakeholder={s} />))}</div>)}
            {viewMode === "list" && loadingMore && (<div className="flex items-center justify-center gap-3 mt-6 px-5 py-4 rounded" style={{ background: "var(--navy-800)", border: "1px solid rgba(255,255,255,0.06)" }}><div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: "var(--crimson)", borderTopColor: "transparent" }} /><p className="text-sm" style={{ color: "#8892a4" }}>Discovering additional regional and local stakeholders{"\u2026"}</p></div>)}

            {viewMode === "map" && (<Suspense fallback={<div className="flex items-center justify-center rounded" style={{ height: "600px", background: "var(--navy-800)", border: "1px solid rgba(255,255,255,0.06)" }}><div className="w-6 h-6 rounded-full border-2 animate-spin" style={{ borderColor: "var(--crimson)", borderTopColor: "transparent" }} /></div>}><div style={{ animation: "fadeIn 0.25s ease-out" }}><MapView stakeholders={filteredStakeholders ?? []} />{loadingMore && (<div className="flex items-center justify-center gap-3 mt-4 px-5 py-3 rounded" style={{ background: "var(--navy-800)", border: "1px solid rgba(255,255,255,0.06)" }}><div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: "var(--crimson)", borderTopColor: "transparent" }} /><p className="text-sm" style={{ color: "#8892a4" }}>Discovering additional stakeholders{"\u2026"}</p></div>)}</div></Suspense>)}
          </>
        )}

        {!loading && !uploadLoading && !stakeholders && !error && (<div className="text-center py-20"><div className="inline-flex items-center justify-center w-16 h-16 rounded mb-4" style={{ background: "rgba(204, 41, 54, 0.08)", border: "1px solid rgba(204, 41, 54, 0.2)" }}><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "var(--crimson)" }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg></div><p className="text-base font-semibold text-white mb-1">No stakeholders mapped yet</p><p className="text-sm" style={{ color: "#8892a4" }}>Enter a sector and region above to begin your analysis</p></div>)}
      </main>

      <footer className="px-8 py-4 text-xs text-center" style={{ color: "#4a5568", borderTop: "1px solid rgba(255,255,255,0.04)" }}>Powered by Claude AI · Stakeholder Intelligence Platform</footer>

      {showAddModal && (<AddStakeholderModal onAdd={(newStakeholder) => { setStakeholders((prev) => prev ? [...prev, newStakeholder] : [newStakeholder]); if (!resultMode) setResultMode("live"); if (!lastQuery) setLastQuery({ sector: "Manual Entry", region: "Various" }); }} onClose={() => setShowAddModal(false)} />)}
    </div>
  );
}
